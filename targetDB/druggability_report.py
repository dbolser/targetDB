...
def get_single_excel(target):
    ...
    for uniprot_id in target.uniprot_ids:
        if uniprot_id in list_of_entries.index:
            output_name = Path(output_single_path).joinpath(
                target.symbol + "_" + uniprot_id + ".xlsx"
            )
            ...
            red = workbook.add_format(
                {"bold": True, "valign": "vcenter", "color": "red"}
            )
            green = workbook.add_format(
                {"bold": True, "valign": "vcenter", "color": "green"}
            )
            ...
            if not pubmed.empty:
                ...
                pubmed.sort_values(
                    by="Year of Publication", ascending=False, inplace=True
                )
                pubmed.to_excel(writer, sheet_name="Pubmed_search", index=False)
                for col_num, value in enumerate(pubmed.columns.values):
                    writer.sheets["Pubmed_search"].write(
                        0, col_num, value, vert_col_header
                    )

            # ========================== OPENTARGET TAB ===============================#
            if not res["open_target"].empty:
                res["open_target"].to_excel(
                    writer, sheet_name="open_target_association", index=False
                )
                for col_num, value in enumerate(res["open_target"].columns.values):
                    writer.sheets["open_target_association"].write(
                        0, col_num, value, vert_col_header
                    )
            ...
            header_index = {
                ...
                "Target_id": (2, 0),
                "Protein_class": (3, 0),
                "Protein_class_desc": (4, 0),
                "Species": (5, 0),
                "Number_isoforms": (6, 0),
                "Tractable": (0, 3),
                "Tractability_probability": (0, 4),
                "In Training data set ?": (2, 3, 2, 4),
                "DISEASE": (8, 0, 8, 1),
                "disease_id": (9, 0),
                "disease_name": (9, 1),
                "PATHWAYS": (8, 3, 8, 4),
            ...
            if not res["general_info"].empty:
                ...
                drug_proba = pd.DataFrame(
                    dml.predict_prob(ml_model, tscore.score_components),
                ...
                tscore.scores["Tractable"] = tscore.scores["Tractable"].replace(
                    {0: "False", 1: "True"}
                )
                tscore.scores["In_training_set"] = dml.in_training_set(
                    tscore.score_components
                )
                target_desc = target_desc.merge(
                    tscore.scores, on="Target_id", how="left"
                )
                score_col = [
                    "structure_info_score",
                    "chemistry_score",
                    "biology_score",
                    "disease_score",
                    "genetic_score",
                ...
                spider_plot = td.make_spider_plot_v3(
                    target_score.loc[uniprot_id].values,
                    target_qual.loc[uniprot_id].to_dict(),
                ...
                wb_general_info.insert_image(
                    "G1", "spider_plot", {"image_data": spider_plot}
                )

                wb_general_info.write(1, 3, target_desc["Tractable"].iloc[0], hv_center)
                wb_general_info.write(
                    1, 4, target_desc["Tractability_probability"].iloc[0], hv_center
                )
                wb_general_info.merge_range(
                    3, 3, 3, 4, target_desc["In_training_set"].iloc[0], hv_center
                )
            ...
            if not res["disease"].empty:
                for i in range(len(res["disease"])):
                    for k, v in res["disease"].iloc[i].items():
                        ...
                        row = row + i + 1
                        if k == "disease_id":
                            wb_general_info.write_url(
                                row,
                                col,
                                "https://omim.org/entry/" + v.split(":")[1],
                                link,
                            )
            ...
            if not res["reactome"].empty:
                ...
                row = row + 1
                wb_general_info.write_column(
                    row, col, list(res["reactome"]["pathway_name"]), v_center
                )
            ...
            if not res["kegg"].empty:
                ...
                row = row + 1
                wb_general_info.write_column(
                    row, col, list(res["kegg"]["pathway_name"]), v_center
                )
            ...
            if not res["gwas"].empty:
                for i in range(len(res["gwas"])):
                    for k, v in res["gwas"].iloc[i].items():
                        ...
                        row = row + i + 1
                        if k == "pubmed_id":
                            wb_disease.write_url(
                                row,
                                col,
                                "https://www.ncbi.nlm.nih.gov/pubmed/" + v,
                                link,
                            )
            ...
            if not res["selectivity"].empty:
                wb_expression.merge_range(
                    0,
                    2,
                    0,
                    3,
                    res["selectivity"].iloc[0]["Selectivity_entropy"],
                    col_header,
                )
            ...
            if not res["tissue_expression"].empty:
                ...
                for i in range(len(res["tissue_expression"])):
                    if res["tissue_expression"].iloc[i]["organ"] != previous_organ:
                        ...
                        wb_expression.merge_range(
                            row, col + 1, row, col + 2, "Cell type", col_header
                        )
                    ...
                    row += 1
                    wb_expression.write(
                        row, col, res["tissue_expression"].iloc[i]["tissue"]
                    )
                    wb_expression.merge_range(
                        row,
                        col + 1,
                        row,
                        col + 2,
                        res["tissue_expression"].iloc[i]["cell"],
                    )
                    wb_expression.write(
                        row, col + 3, res["tissue_expression"].iloc[i]["value"]
                    )
            ...
            if not res["phenotype"].empty:
                ...
                normal_phen = re.compile(
                    "(no abnormal phenotype detected)", re.IGNORECASE
                )
                ...
                for allele, data in res["phenotype"].groupby(["Allele_symbol"]):
                    ...
                    for zygosity, d2 in data.groupby(["zygosity"]):
                        for genotype, d3 in d2.groupby(["genotype"]):
                            ...
                            if lethal and normal:
                                wb_genotypes.write(
                                    row + 3, col_gen, genotype[0], col_header_orangebg
                                )
                            elif lethal:
                                wb_genotypes.write(
                                    row + 3, col_gen, genotype[0], col_header_redbg
                                )
                            elif normal:
                                wb_genotypes.write(
                                    row + 3, col_gen, genotype[0], col_header_greenbg
                                )
                            else:
                                wb_genotypes.write(
                                    row + 3, col_gen, genotype[0], col_header
                                )

                            col_gen += 1
                        if col_gen - col_zyg == 1:
                            wb_genotypes.write(
                                row + 2, col_zyg, zygosity[0], col_header
                            )
                        else:
                            wb_genotypes.merge_range(
                                row + 2,
                                col_zyg,
                                row + 2,
                                col_gen - 1,
                    ...
                    if col_zyg - col_allele == 1:
                        ...
                        wb_genotypes.write(row, col_allele, allele[0], col_header)
                    else:
                        wb_genotypes.merge_range(
                            row + 1,
                            col_allele,
                        ...
                        wb_genotypes.merge_range(
                            row, col_allele, row, col_zyg - 1, allele[0], col_header
                        )
                    col_allele = 0
                    col_zyg = 0
                    col_gen = 0
            ...
            if not res["isoforms"].empty:
                ...
                for iso in res["isoforms"].to_dict(orient="records"):
                    wb_isoforms.merge_range(
                        row, 0, row, 6, iso["isoform_name"], col_header
                    )
                    row += 1
                    ...
                    if not res["isoforms_mod"].empty:
                        ...
                        row += 1
                        to_print = res["isoforms_mod"][
                            res["isoforms_mod"]["isoform_id"] == iso["Isoform_id"]
                        ]
                        to_print = to_print[
                            [
                                "start",
            ...
            if not res["var"].empty:
                ...
                row = 0
                wb_var_mut.merge_range(
                    row, col_var, row, col_var + 6, "VARIANTS", col_header
                )
                row += 1
                for header, col in mod_header.items():
                ...
                for i in range(len(res["var"])):
                    ...
                    for key, value in res["var"].iloc[i].items():
                        if key == "previous_seq" or key == "new_seq":
                            wb_var_mut.write(
                                row, col_var + mod_header[key], value, wrap
                            )
                        else:
            ...
            if not res["mut"].empty:
                ...
                if not res["var"].empty:
                    ...
                    row = 0
                wb_var_mut.merge_range(
                    row, col_mut, row, col_mut + 6, "MUTANTS", col_header
                )
                row += 1
                for header, col in mod_header.items():
                ...
                for i in range(len(res["mut"])):
                    ...
                    for key, value in res["mut"].iloc[i].items():
                        if key == "previous_seq" or key == "new_seq":
                            wb_var_mut.write(
                                row, col_mut + mod_header[key], value, wrap
                            )
                        else:
            ...
            if sequence:
                wb_struct.merge_range(
                    row, col_orig, row, col_orig + 4, "Total length", col_header
                )
                row += 1
                wb_struct.merge_range(
                    row, col_orig, row, col_orig + 4, len(sequence), bold_center
                )
                row += 2

            if not res["domains"].empty:
                wb_struct.merge_range(
                    row, col_orig, row, col_orig + 4, "DOMAINS", col_header
                )
                row += 1
                col_order = ["Domain_name", "start", "stop", "length", "source"]
                res["domains"] = res["domains"][col_order]
                res["domains"].to_excel(
                    writer, sheet_name="Structure", startrow=row, index=False
                )
            ...
            if not res["pdb_blast"].empty:
                wb_struct.merge_range(
                    row, col_orig, row, col_orig + 6, "PDB BLAST", col_header
                )
                row += 1
                col_order = [
                    "PDB_code",
            ...
            if not res["pdb"].empty:
                ...
                row = 0
                wb_struct.merge_range(
                    row, col_orig, row, col_orig + 7, "PDB", col_header
                )
                wb_struct.merge_range(
                    row, col_orig + 8, row, col_orig + 15, "PDB: Ligand", col_header
                )
                wb_struct.merge_range(
                    row,
                    col_orig + 16,
                    row,
                    col_orig + 17,
                    "ChEMBL - DruggEbillity",
                    col_header,
                )
                row += 1

                pdb = res["pdb"].copy()
                pdb["% of full protein"] = round(
                    (pdb["n_residues"] / len(sequence)) * 100, 1
                )
                pdb.operator = " " + pdb.operator
                pdb["PDBbind_link"] = pdb["PDB_code"].apply(
                    lambda x: '=HYPERLINK("http://www.pdbbind.org.cn/quickpdb.asp?quickpdb='
                    + x
                    + '","'
                    + x
                    + '")'
                ...
                pdb.loc[
                    pdb["type_of_binder"].isna() | (pdb["type_of_binder"] == ""),
                    "PDBbind_link",
                ] = ""
                col_order = [
                    "PDB_code",
            ...
            if not res["pockets"].empty:
                ...
                res["pockets"].to_excel(
                    writer, sheet_name="Pockets", startrow=1, index=False
                )
            ...
            if not res["alt_pockets"].empty:
                ...
                wb_pockets = writer.sheets["Pockets"]
                wb_pockets.merge_range(
                    0,
                    0 + col_alt_pocket,
                    0,
                    9 + col_alt_pocket,
                    "ALTERNATE DRUGGABLE POCKETS (PDB from blast)",
                    col_header,
                )

            # =================== CHEMBL BIOACTIVITIES TABS ===========================#
            header_groups = {
                "Bioactivity info": (0, 9),
                "Assay info": (10, 14),
                "Structure": (15, 15),
                "Ligand properties": (16, 30),
                "Ligand info": (31, 36),
                "References": (37, 38),
            }
            CNS_MPO_criteria = [
                {"criteria": ">=", "type": "number", "value": 4.5},
            ...
            if not res["binding"].empty:
                res["binding"].to_excel(
                    writer, sheet_name="Binding", startrow=1, index=False
                )
                ...
                w_bd.conditional_format(
                    CNS_MPO_col + (str(len(res["binding"]) + 3)),
                    {
                        "type": "icon_set",
                        "icon_style": "3_traffic_lights",
            ...
            if not res["dose_response"].empty:
                res["dose_response"].to_excel(
                    writer, sheet_name="Dose_response", startrow=1, index=False
                )
                ...
                w_dr.conditional_format(
                    CNS_MPO_col + (str(len(res["dose_response"]) + 3)),
                    {
                        "type": "icon_set",
                        "icon_style": "3_traffic_lights",
                        "icons": CNS_MPO_criteria,
                ...
                for col_num, value in enumerate(res["dose_response"].columns.values):
                    writer.sheets["Dose_response"].write(
                        1, col_num, value, vert_col_header
                    )

            if not res["percent_inhibition"].empty:
                res["percent_inhibition"].to_excel(
                    writer, sheet_name="Percent_inhibition", startrow=1, index=False
                )
                ...
                for col_num, value in enumerate(
                    res["percent_inhibition"].columns.values
                ):
                    writer.sheets["Percent_inhibition"].write(
                        1, col_num, value, vert_col_header
                    )
                w_per.conditional_format(
                    CNS_MPO_col + (str(len(res["percent_inhibition"]) + 3)),
                    {
                        "type": "icon_set",
                        "icon_style": "3_traffic_lights",
                        "icons": CNS_MPO_criteria,
            ...
            if not res["efficacy_bio"].empty:
                res["efficacy_bio"].to_excel(
                    writer, sheet_name="Emax_Efficacy", startrow=1, index=False
                )
                ...
                for col_num, value in enumerate(res["efficacy_bio"].columns.values):
                    writer.sheets["Emax_Efficacy"].write(
                        1, col_num, value, vert_col_header
                    )
                w_eff.conditional_format(
                    CNS_MPO_col + (str(len(res["efficacy_bio"]) + 3)),
                    {
                        "type": "icon_set",
                        "icon_style": "3_traffic_lights",
                        "icons": CNS_MPO_criteria,
                ...
                row_efficacy = (
                    len(res["efficacy_bio"]) + len(res["efficacy_bio"].columns) + 1
                )
            else:
                row_efficacy = 0
            if not res["emax"].empty:
                res["emax"].to_excel(
                    writer,
                    sheet_name="Emax_Efficacy",
                    startrow=row_efficacy + 1,
                    index=False,
                )
                w_eff = writer.sheets["Emax_Efficacy"]
                for head, span in header_groups.items():
                ...
                for col_num, value in enumerate(res["emax"].columns.values):
                    writer.sheets["Emax_Efficacy"].write(
                        row_efficacy + 1, col_num, value, vert_col_header
                    )
                w_eff.conditional_format(
                    CNS_MPO_col + (str(len(res["emax"]) + row_efficacy + 3)),
                    {
                        "type": "icon_set",
                        "icon_style": "3_traffic_lights",
                        "icons": CNS_MPO_criteria,
            ...
            if not res["ADME"].empty:
                ...
                w_adme.conditional_format(
                    CNS_MPO_col + (str(len(res["ADME"]) + 3)),
                    {
                        "type": "icon_set",
                        "icon_style": "3_traffic_lights",
                        "icons": CNS_MPO_criteria,
            ...
            if not res["other"].empty:
                res["other"].to_excel(
                    writer, sheet_name="Other_bioactivities", startrow=1, index=False
                )
                ...
                for col_num, value in enumerate(res["other"].columns.values):
                    writer.sheets["Other_bioactivities"].write(
                        1, col_num, value, vert_col_header
                    )
                w_other.conditional_format(
                    CNS_MPO_col + (str(len(res["other"]) + 3)),
                    {
                        "type": "icon_set",
                        "icon_style": "3_traffic_lights",
                        "icons": CNS_MPO_criteria,
            ...
            if not res["bindingDB"].empty:
                ...
                bdb.loc[
                    (bdb[["LogP", "LogD", "MW", "HBD", "TPSA"]].notnull().all(axis=1))
                    & (bdb.bpKa.isnull()),
                    "bpKa",
                ] = 0
                bdb["CNS_MPO"] = mpo.calc_mpo_score(
                    bpka=bdb["bpKa"],
                    logP=bdb["LogP"],
                    logD=bdb["LogD"],
                    MW=bdb["MW"],
                ...
                if not bdb.empty:
                    ...
                    CNS_MPO_col = "AD1:AD"
                    w_bindingDB.conditional_format(
                        CNS_MPO_col + (str(len(bdb) + 3)),
                        {
                            "type": "icon_set",
                            "icon_style": "3_traffic_lights",
                            "icons": CNS_MPO_criteria,
            ...
            if not res["commercials"].empty:
                ...
                comm = pd.merge(
                    comm,
                    comm.website.str.split(" ", expand=True).add_prefix("Source_"),
                    left_index=True,
                    right_index=True,
                )
                ...
                for col_num, value in enumerate(comm.columns.values):
                    writer.sheets["Commercial compounds"].write(
                        0, col_num, value, vert_col_header
                    )
...
def get_list_excel(list_targets, not_found=[]):
    ...
    if pubmed_email:
        ...
        for gene_symbol in list_targets.index:
            ...
            pubmed["number of Dementia publications"].extend(
                [
                    pubmed_search(
                        list_targets.symbol.loc[gene_symbol],
                        pubmed_email,
                        return_number=True,
                        mesh_term="Dementia",
                    )
                ]
                * len(list_targets.uniprot_ids.loc[gene_symbol])
            )
    ...
    druggability_pred = dml.predict(ml_model, tscore.score_components)
    drug_proba = pd.DataFrame(
        dml.predict_prob(ml_model, tscore.score_components), columns=ml_model.classes_
    )
    ...
    weights_dict = {
        "structure_info_score": "structure_info_score [mpo coeff= "
        + str(tscore.coeff["sbio"])
        + "]",
        "structural_drug_score": "structural_drug_score [mpo coeff= "
        + str(tscore.coeff["sdrug"])
        + "]",
        "chemistry_score": "chemistry_score [mpo coeff= "
        + str(tscore.coeff["chem"])
        + "]",
        ...
        "information_score": "information_score [mpo coeff= "
        + str(tscore.coeff["info"])
        + "]",
    ...
    output_file_name = Path(output_lists_path).joinpath(
        "Export_" + str(len(data)) + "_entries_" + t + ".xlsx"
    )
    ...
    col_order = [
        ...
        "PDB_total_count",
        "PDB_with_Ligand_count",
        "%_sequence_covered",
        "%_domain_covered",
        "PDB_sites_tractable_count",
    ...
    data.sort_values(
        "mpo_score", axis=0, ascending=False, inplace=True, na_position="last"
    )
    ...
    header_groups = {
        "GENERAL INFO": (0, gen_info_len - 1),
        "SCORES": (gen_info_len, gen_info_len + score_len - 1),
        "LITTERATURE/PATENT INFORMATION": (
            gen_info_len + score_len,
            gen_info_len + score_len + litt_len - 1,
        ),
        "BIOLOGY": (
            gen_info_len + score_len + litt_len,
            gen_info_len + score_len + litt_len + bio_len - 1,
        ),
        "PATHWAYS AND DISEASES": (
            gen_info_len + score_len + litt_len + bio_len,
            gen_info_len + score_len + litt_len + bio_len + pathways_len - 1,
        ),
        "STRUCTURAL INFORMATION": (
            gen_info_len + score_len + litt_len + bio_len + pathways_len,
            gen_info_len
            + score_len
            + litt_len
            + bio_len
    ...
    color_dict = {
        "GENERAL INFO": "#fde9d9",
        "SCORES": "#ffff99",
        "LITTERATURE/PATENT INFORMATION": "#d9d9d9",
        "BIOLOGY": "#ebf1de",
        "PATHWAYS AND DISEASES": "#f2dcdb",
        "STRUCTURAL INFORMATION": "#dce6f1",
        "CHEMISTRY": "#e4dfec",
    }
    for head, span in header_groups.items():
        col_header = workbook.add_format(
        ...
        if span[0] == span[1]:
            ...
            writer.sheets["Druggability_list"].merge_range(
                0, span[0], 0, span[1], head, col_header
            )
...
def parse_args():
    ...
    if not args.report_list and not args.report_single:
        print(
            'ERROR: Please provide a report type "-rl" for a list report, "-rs" for a single target report'
        )
        parser.print_help()
        sys.exit()
    return args
        ...
        if config_file_path.is_file() and not update_config:
            ...
            for var_name in config["output_path"]:
                if var_name in ["db_files"]:
                    continue
                if (
                    not Path(config["output_path"][var_name]).is_dir()
                    or config["output_path"][var_name] == ""
                ):
                    todo.append(var_name)
            ...
            config = cf.get_config_from_user(
                config, todo=["list", "single", "targetdb", "email"], new=True
            )
            with config_file_path.open(mode="w") as cfile:
                config.write(cfile)
            update_config = False

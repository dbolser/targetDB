...
def get_crossref_pdb_go_chembl(record):
    ...
    PDB_list = pd.DataFrame(
        columns=["PDB_code", "Technique", "Resolution", "Chain", "Domain", "path"]
    )
    chembl_id = None
    GO_process = []
    GO_Function = []
...
def get_humanmine_data(gene):
    ...
    if query.count() == 1:
        ...
        differential_exp_tissues_df = pd.DataFrame(
            columns=["T_statistic", "Tissue", "expression_status", "p_value"]
        )
        ...
        return (
            disease_df,
            phenotypes_df,
            differential_exp_diseases_df,
            differential_exp_tissues_df,
            gwas_df,
            pathways_df,
        )
        ...
        disease.append(
            {
                ...
                "disease_id": row["diseases.primaryIdentifier"],
            }
        )
    ...
    for row in query.rows():
        phenotypes.append(
            {
                "gene": row["symbol"],
                "organism": row["homologues.homologue.organism.shortName"],
    ...
    for row in query.rows():
        if row["atlasExpression.type"] == "disease_state":
            differential_exp_diseases.append(
                {
                    "T_statistic": row["atlasExpression.tStatistic"],
                    "Condition": row["atlasExpression.condition"],
            ...
            differential_exp_tissues.append(
                {
                    "T_statistic": row["atlasExpression.tStatistic"],
                    "Tissue": row["atlasExpression.condition"],
                    "p_value": row["atlasExpression.pValue"],
                    "expression_status": row["atlasExpression.expression"],
                }
            )
    ...
    for row in query.rows():
        gwas.append(
            {
                "p_value": row["results.pValue"],
                "phenotype": row["results.phenotype"],
                "first_author": row["firstAuthor"],
                "pubmed_id": row["publication.pubMedId"],
    ...
    for row in query.rows():
        pathways.append(
            {
                "pathway_name": row["pathways.name"],
                "pathway_dataset": row["pathways.dataSets.name"],
            }
        )
...
def get_domains(record=None, gene_id=None, chembl_id=None):
    ...
    if chembl_id:
        ...
        query = (
            "SELECT CD.start_position,CD.end_position,DOM.domain_name,DOM.source_domain_id,DOM.domain_type FROM target_dictionary "
        ...
        for i in domains_df.index:
            domain_id = (
                str(gid)
                + str(domains_df.loc[i].start_position)
                + str(domains_df.loc[i].end_position)
                + "_"
                + domains_df.loc[i].source_domain_id
            )
            domain.append(
                {
                    "Start": int(domains_df.loc[i].start_position),
                    "Stop": int(domains_df.loc[i].end_position),
                    "name": domains_df.loc[i].domain_name,
                    "length": int(domains_df.end_position.loc[i])
                    - int(domains_df.start_position.loc[i]),
                    "domain_id": domain_id,
                    "source_name": domains_df.domain_type.loc[i],
        ...
        for i in range(len(domain)):
            for j in range(len(domain)):
                if i == j:
                    ...
                    domain[i]["domain_id"].split("_")[0]
                    == domain[j]["domain_id"].split("_")[0]
                ):
...
def get_chembl_info(chembl_id):
    connector = sqlite3.connect(chembl_24)
    query = (
        "SELECT PC.pref_name ,PC.short_name ,PC.protein_class_desc ,group_concat(DISTINCT("
    ...
    entry_info.rename(
        columns={
            ...
            "pref_name": "Class_name",
            "protein_class_desc": "Class_description",
        },
        inplace=True,
    )
    return entry_info
...
def get_variants(record, domains):
        ...
        if feat[0] in ["VAR_SEQ", "VARIANT"]:
            if str(feat[3]).startswith("Missing"):
                ...
                try:
                    ...
                    comment = "(" + str(feat[3]).split("(")[0]
            modifications.loc[feat[4]] = {
                "mod_id": feat[4],
                "mod_type": str(feat[4]).split("_")[0],
                "start": feat[1],
                "stop": feat[2],
                "previous": previous,
                "new": new,
        ...
        if feat[0] == "MUTAGEN":
            if str(feat[3]).startswith("Missing"):
                ...
                try:
                    ...
                    comment = str(feat[3]).split(":", 1)[0]
                action = "replace"
            modifications.loc["MUTAGEN_" + str(feat[1])] = {
                "mod_id": "MUTAGEN_" + str(feat[1]),
                "mod_type": "MUTAGEN",
                "start": feat[1],
                "stop": feat[2],
                "previous": previous,
    ...
    for i in domains.index:
        for m in modifications.index:
            if (
                modifications.loc[m]["start"] >= domains.Start.loc[i]
                and modifications.loc[m]["stop"] <= domains.Stop.loc[i]
            ):
    ...
    for isoform in isoforms.index:
        ...
        if "Displayed" in isoforms.loc[isoform]["seq_mod"]:
            ...
            isoforms.loc[isoform]["sequence"] = get_uniprot(
                str(isoforms.loc[isoform]["isoid"]).split("-")[0]
            ).sequence
...
def align(sequence1, sequence2, end_gaps=True, print_align=False):
    ...
    if (
        sequence1 == "no_sequence"
        or sequence2 == "no_sequence"
        or sequence1 == ""
        or sequence2 == ""
    ):
        return None
...
def get_pdb(list_of_pdb, path):
    ...
    for k in list_of_pdb.index:
        ...
        if saved_pdb.is_file():
            ...
            try:
                urllib.urlretrieve(
                    "http://files.rcsb.org/download/" + saved_pdb.name, str(saved_pdb)
                )
                list_of_pdb.at[k, "path"] = str(saved_pdb)
                if verbose:
                ...
                if verbose:
                    print(
                        "[ERROR]: "
                        + str(k)
                        + ".pdb file not found (do not exist or is "
                        "too big)"
                    )
    if verbose:
...
def get_pdb_seq_info(pdb_list, domain):
    ...
    for keys in pdb_list.index:
        if pdb_list.path.loc[keys] == "":
            continue
        seq.append(
            pdb_parser.get_sequence(
                keys, pdb_list.loc[keys]["path"], pdb_list.loc[keys]["Chain"], domain
            )
        )
    if verbose:
        print("[PDB INFO]: Done")
...
def get_ligands_to_do(chembl_code):
    ...
    connector = sqlite3.connect(chembl_24)
    query_lig_target = (
        "SELECT TD.chembl_id AS target_id,MOL.chembl_id AS lig_id,version.name as "
    ...
    lig_ids = "("
    query_lig_bioact_db_base = (
        """SELECT DISTINCT lig_id,chembl_version FROM bioactivities WHERE lig_id IN """
    )
...
def get_ligands_info():
    ...
    rename = {
        "cx_logd": "acd_logd",
        "cx_logp": "acd_logp",
        "cx_most_bpka": "acd_most_bpka",
        "cx_most_apka": "acd_most_apka",
    }
...
def get_bioactivity(lig_to_do):
    ...
    if verbose:
        print(
            "[BIOACTIVITIES]: Extracting bioactivities from chembl - CAN TAKE A FEW MINUTES "
        )
    ...
    for i in lig_to_do:
        ...
        if counter == 1000:
            ...
            res_bioactivity = pd.concat(
                [res_bioactivity, pd.read_sql(query_bioactivity, con=connector)]
            )
            counter = 0
            lig_str = ""
    if counter == 0:
...
def pdb_blast(sequence, path, gene_id, gene="", pdb_list=None):
    ...
    if blast_file.is_file():
        if ((((time.time() - blast_file.stat().st_mtime) / 60) / 60) / 24) <= 15:
            ...
            if verbose:
                print(
                    "[3D BLAST FILE FOUND]: File older than 2 weeks, a new blast will be performed ("
                    + gene
                    + ")"
                )
            blast_file.unlink()
            ...
            if blast_file.is_file():
                ...
                print(
                    "[3D BLAST][ERROR]: Something went wrong, no blast result generated"
                )
    ...
    for alignment in blast_record.alignments:
        ...
        for hsp in alignment.hsps:
            ...
            if hsp.score > 200 and hsp.expect < e_value_treshold and similarity > 60:
                data = {
                    "PDB_code": pdb_code,
                    "seq_percent": percent_seq,
                    "similarity": similarity,
                    "Chain_id": chain_id,
                    "chain_letter": chain_id.split("_")[1],
                    "gene": "",
                    "organism": "",
                    "uniprot_id": "",
                    "path": "",
    ...
    for id in alternate_pdb.index:
        if Path(alternate_pdb.path.loc[id]).is_file():
            result = pdb_to_uniprot(
                alternate_pdb.loc[id]["chain_letter"], alternate_pdb.loc[id]["path"]
            )
            if result:
...
def proteins_blast(sequence, gene_id, gene, path):
    ...
    if blast_file.is_file():
        if ((((time.time() - blast_file.stat().st_mtime) / 60) / 60) / 24) <= 15:
            ...
            if verbose:
                print(
                    "[PROTEIN BLAST FILE FOUND]: File older than 2 weeks, a new blast will be performed ("
                    + gene
                    + ")"
                )
            blast_file.unlink()
            ...
            if blast_file.is_file():
                ...
                print(
                    "[PROTEIN BLAST][ERROR]: Something went wrong, no blast result generated"
                )
                return []
    else:
        blast_launcher(sequence, seq_file, "swissprot", blast_file, num_core=bcore)
    ...
    if verbose:
        print(
            "[PROTEIN BLAST DONE]: Now parsing the data - " + gene + "(" + gene_id + ")"
        )
    e_value_treshold = 0.0001
    query_length = float(blast_record.query_length)
    ...
    for alignment in blast_record.alignments:
        for hsp in alignment.hsps:
            ...
            idx_name = [
                idx for idx, s in enumerate(alignment.hit_def.split(":")) if "Full" in s
            ][0]
            neighbour_gene_name = (
                alignment.hit_def.split(":")[idx_name].split("=")[1].split(";")[0]
            )
            ...
            if (
                hsp.score > 200
                and hsp.expect < e_value_treshold
                and (length / query_length) > 0.5
                and similarity >= 40
                and neighbour_accession_code != gene_id
            ):
                list_of_neighbours.loc[gene_id + "_" + neighbour_accession_code] = {
                    "Query_target_id": gene_id,
...
def pubmed_search(gene_name, email, return_number=False, mesh_term=None):
    ...
    info_pub = Entrez.efetch(
        db="pubmed", id=pid["IdList"], rettype="medline", retmode="text"
    )
    ...
    for i in df["MeSH Terms"].values:
        if type(i) == float:
            ...
            for k in i:
                if (
                    "neurodege" in k.lower()
                    or "alzheimer" in k.lower()
                    or "dementia" in k.lower()
                    or "parkinson" in k.lower()
                ):
                    neuro = True
                if "*" in k:
                    major.append(k)
...
def open_target_association(ensembl_id):
    ...
    try:
        ...
        r = requests.post(
            base_url, json={"query": query_string, "variables": {}}, timeout=20
        )
    ...
    ad = pd.DataFrame.from_dict(
        api_response_as_json["data"]["target"]["associatedDiseases"]
    )
    # Extract the approved gene symbol for this target
    gene_symbol = pd.DataFrame.from_dict(api_response_as_json["data"]["target"])[
        "approvedSymbol"
    ][1]
...
def write_to_db(target, db_path):
    ...
    target.domain.to_sql(
        "Domain_targets", con=connector, index=False, if_exists="append"
    )
    ...
    target.alternate_pdb.rename(
        columns={
            "PDB_code": "Hit_PDB_code",
            "Chain_id": "Hit_Chain_id",
            "chain_letter": "Chain_Letter",
            "gene": "Hit_gene_name",
            "organism": "Hit_gene_species",
            "uniprot_id": "Hit_gene_id",
        },
        inplace=True,
    )
    target.alternate_pdb.drop(columns=["seq_percent", "path"], inplace=True)
    target.alternate_pdb["Unique_ID"] = (
        target.alternate_pdb.Query_target_id
        + "_"
        + target.alternate_pdb.Hit_gene_id
        + "_"
        + target.alternate_pdb.Hit_Chain_id
    )
    target.alternate_pdb.to_sql(
        "3D_Blast", con=connector, index=False, if_exists="append"
    )
    ...
    target.neighbours.to_sql(
        "protein_blast", con=connector, index=False, if_exists="append"
    )
    ...
    target.modifications["Unique_modID"] = (
        target.swissprotID + "_" + target.modifications.mod_id
    )
    target.modifications["domains"] = [
        ",".join(map(str, l)) for l in target.modifications["domains"]
    ]
    target.modifications.to_sql(
        "modifications", con=connector, index=False, if_exists="append"
    )
        ...
        for j in target.isoforms.seq_mod.loc[i]:
            if j in ("Displayed", "External"):
                ...
                isoforms_mods.append(
                    {
                        "mod_id": target.swissprotID + "_" + j,
                        "isoform_id": target.isoforms.isoid.loc[i],
                    }
                )
    isoform_mods = pd.DataFrame.from_records(isoforms_mods)
    target.isoforms["Target_id"] = target.swissprotID
    target.isoforms.rename(
        columns={"isoid": "Isoform_id", "name": "Isoform_name", "sequence": "Sequence"},
        inplace=True,
    )
    ...
    isoform_mods.to_sql(
        "isoform_modifications", con=connector, if_exists="append", index=False
    )
    ...
    if not target.pdb_info.empty:
        ...
        target.pdb_info.equal = target.pdb_info.equal.apply(
        ...
        target.pdb_info.to_sql(
            "PDB_Chains", con=connector, if_exists="append", index=False
        )

    pdb_domains.to_sql(
        "PDBChain_Domain", con=connector, if_exists="append", index=False
    )
    ...
    if not target.pockets["pockets_chain"].empty:
        target.pockets["pockets_chain"].List_of_contacts = target.pockets[
            "pockets_chain"
        ].List_of_contacts.apply(lambda x: ",".join(x))
        target.pockets["pockets_chain"].to_sql(
            "fPockets_Chain", con=connector, if_exists="append", index=False
        )

    target.pockets["pockets"].to_sql(
        "fPockets", con=connector, if_exists="append", index=False
    )
    target.pockets["pockets_domain"].to_sql(
        "fPockets_Domain", con=connector, if_exists="append", index=False
    )

    if verbose:
        print("[DATABASE]: fPockets tables populated/updated")

    target.alternate_pockets["pockets"].to_sql(
        "fPockets", con=connector, if_exists="append", index=False
    )
    ...
    target.bioactivities.to_sql(
        "bioactivities", con=connector, if_exists="append", index=False
    )
    ...
    if target.protein_expression is not None:
        protein_level = pd.DataFrame.from_records(
            target.protein_expression.protein_lvl["Cells"]
        )
        protein_selectivity = pd.DataFrame(
            {
                "Selectivity_entropy": [target.protein_expression.selective_entropy],
        ...
        protein_level.rename(
            columns={"cell_id": "Entry_id", "level": "value", "cell_type": "cell"},
            inplace=True,
        )

        protein_level.to_sql(
            "protein_expression_levels", con=connector, if_exists="append", index=False
        )
        protein_selectivity.to_sql(
            "protein_expression_selectivity",
            con=connector,
            if_exists="append",
            index=False,
        )
    ...
    target.differential_exp_tissues.rename(
        columns={"T_statistic": "t_stat"}, inplace=True
    )
    target.differential_exp_tissues.to_sql(
        "diff_exp_tissue", con=connector, if_exists="append", index=False
    )
    ...
    target.differential_exp_disease.rename(
        columns={"T_statistic": "t_stat", "Condition": "disease"}, inplace=True
    )
    target.differential_exp_disease.to_sql(
        "diff_exp_disease", con=connector, if_exists="append", index=False
    )
    ...
    target.phenotypes.to_sql(
        "phenotype", con=connector, if_exists="append", index=False
    )
    ...
    target.open_targets.to_sql(
        "opentarget_association", con=connector, if_exists="append", index=False
    )
...
class Target:
    def __init__(
        ...
        self.pockets = {
            ...
            "pockets_chain": pd.DataFrame(),
            "pockets_domain": pd.DataFrame(),
        }
        self.record = None
        self.pubmed = None
        self.pathways = []
        ...
        while True:
            ...
            if self.record is None:
                if verbose:
                    print(
                        "[GENE SKIPPED]: No Uniprot data or wrong uniprotID for "
                        + self.gene
                    )
                break
            ...
            self.protein_expression = patlas.ProteinExpression(
                self.gene, id=self.ensembl_id
            )
            ...
            (
                ...
                self.gwas,
                self.pathways,
            ) = get_humanmine_data(self.gene)
            # ==========# GET DOMAIN INFORMATION FROM BOTH CHEMBL AND UNIPROT #===========#
            self.domain = get_domains(
                record=self.record, gene_id=self.swissprotID, chembl_id=self.chembl_id
            )
            ...
            self.prot_info.rename(
                columns={
                    "Synonym": "Synonyms",
                    "Class_name": "Protein_class",
                    "Class_short": "Protein_class_short",
                    "Class_description": "Protein_class_desc",
                },
                inplace=True,
            )
            ...
            query_pdb = (
                "SELECT PDB_code FROM PDB_Chains WHERE Target_id ='%s' GROUP BY PDB_code"
                % self.swissprotID
            )
            query_pockets = (
                "SELECT Pocket_id FROM fPockets WHERE Target_id ='%s' AND druggable='TRUE' AND blast='FALSE'"
                % self.swissprotID
            )
            ...
            if not self.pdb.empty:
                ...
                self.pockets = pocket.get_pockets(
                    self.path,
                    sphere_size=3,
                    pdb_info=self.pdb,
                    domain=self.domain,
                    uniprot_id=self.swissprotID,
                    fpocket_exe=fpocket_exe,
                    verbose=verbose,
                )
                self.druggable_pockets = self.pockets["pockets"][
                    self.pockets["pockets"].druggable == "TRUE"
                ].copy()
            ...
            if pd.read_sql("SELECT assay_id FROM assays", con=connector).empty:
                if verbose:
                    print(
                        "[INFO]: Assay table is being filled, it may take a few minutes"
                    )
                get_assays()
                if verbose:
            ...
            if pd.read_sql("SELECT lig_id FROM ligands", con=connector).empty:
                if verbose:
                    print(
                        "[INFO]: Ligands table is being filled, it may take a few minutes"
                    )
                get_ligands_info()
                if verbose:
            ...
            # ============================================================================#
            self.alternate_pdb = pdb_blast(
                self.sequence,
                self.path,
                self.swissprotID,
                pdb_list=pdb_super_list,
                gene=self.gene,
            )
            very_close_pdb = self.alternate_pdb[
                self.alternate_pdb.similarity >= 90.0
            ].copy()

            if res_pockets.empty and self.druggable_pockets.empty:
                self.alternate_pockets = pocket.get_pockets(
                    self.path,
                    sphere_size=3,
                    alternate=True,
            ...
            self.neighbours = proteins_blast(
                self.sequence, self.swissprotID, self.gene, self.path
            )
            # ============================================================================#
        ...
        if verbose:
            print(
                ...
                + self.gene
                + " (in "
                + str(round(script_stop - script_start))
                + " sec)"
            )
            print(
                "======================================================================="
...
def parse_args():
    ...
    parser.add_argument(
        "-l",
        "--list_genes",
        help='Enter a list of gene name separated by a ","',
        metavar="",
    )
    parser.add_argument(
        "-a",
        "--do_all",
        help="Use this option to create a database with all human genes (list coming from HGNC)",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "-s",
        "--sphere_size",
        help="enter a value for the probe size of the pocket finder tool ("
        "default = 3.0)",
        metavar="",
        type=float,
        default=3.0,
    ...
    parser.add_argument(
        "-v", "--verbose", help="Print information", action="store_true", default=False
    )

    parser.add_argument(
        "-update",
        "--update",
        help="Update record if already in database (default: No)",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "-blastcore",
        "--num_core",
        help="Enter the value of processor core to use for the blast search (default=8)",
        metavar="",
        type=int,
        default=8,
    )
    ...
    parser.add_argument(
        "-create_db",
        "--create_db",
        help="Use this option to create an empty targetDB database",
        action="store_true",
        default=False,
    )
    ...
    if (
        not arguments.gene
        and not arguments.in_file
        and not arguments.list_genes
        and not arguments.do_all
    ):
        print(
            "[ERROR]: Please use one of the optional input options : -g / -i / -l / -a"
        )
        parser.print_help()
        sys.exit()
    print(arguments)
...
def main():
    ...
    while True:
        ...
        if config_file_path.is_file() and not update_config:
            ...
            if not Path(config["executables"]["fpocket"]).is_file() and os.name != "nt":
                todo.append("fpocket")
            if (
                not Path(config["executables"]["blast"]).is_file()
                or not Path(config["executables"]["blastdb_path"]).is_dir()
            ):
                todo.append("blast")
            if (
                not Path(config["output_path"]["db_files"]).is_dir()
                or config["output_path"]["db_files"] == ""
            ):
        ...
        if args.in_file:
            if Path(args.in_file).is_file():
                with open(args.in_file, "r") as gene_list:
                    ...
                    gene_df, missing = g2id.gene_to_id(
                        list_of_genes, targetDB_path=targetDB
                    )
                break
            else:
    ...
    for g_id in gene_df.index:
        if len(gene_df.uniprot_ids.loc[g_id]) == 0:
            if args.verbose:
                print(
                    "[GENE SKIPPED]: No uniprot id was found for the entered gene name: ",
                    gene_df.symbol.loc[g_id],
                )
        for uniprot_id in gene_df.uniprot_ids.loc[g_id]:
            ...
            if uniprot_id in entries_in_db.index:
                if args.update:
                    if args.verbose:
                        print(
                            "[GENE INFO]: Target already present in database, updating informations"
                        )
                    activate_fk_sql = """PRAGMA foreign_keys = 1"""
                    delete_sql = (
                        """DELETE FROM Targets WHERE Target_id='%s'""" % uniprot_id
                    )
                    ...
                    Target(
                        ...
                        ensembl_id=gene_df.ensembl_gene_id.loc[g_id],
                        hgnc_id=g_id,
                        db_files_path=dbase_file_path,
                        chembl=chembl_24,
                        target_db=targetDB,
                        blast_cores=args.num_core,
                        v=args.verbose,
                    )
                else:
                    if args.verbose:
                        print(
                            "[GENE SKIPPED]: Already present in the database: "
                            + gene_df.symbol.loc[g_id]
                        )
                        print(
                            "======================================================================="

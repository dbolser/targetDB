...
def open_db(database, pwd=None, user=None):
    db = MySQLdb.connect(
        "localhost", user, pwd, database, charset="utf8", use_unicode=True
    )
...
class db_connector:
    ...
    def connect(self, database):
        self.db = MySQLdb.connect(
            "localhost", self.user, self.pwd, database, charset="utf8", use_unicode=True
        )
    ...
    def multi_write(self, query, values, update=""):
        ...
        for i in range(len(values)):
            ...
            count += 1
            if count % limit == 0:
                values_to_query = tup_value[
                    (n_loop * limit) * n_column : ((n_loop * limit) * n_column)
                    + (limit * n_column)
                ]

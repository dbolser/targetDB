    ...
    if name:
        ...
        try:
            ...
            if len(from_to) > 1:
                ...
                to_res = int(from_to[0])
        except ValueError:
            try:
                from_to = line[10:].strip().split(":")
                if len(from_to) > 1:
                    from_to_2 = from_to[1].split("..")
                    if len(from_to_2) > 1:
                        from_res = int(from_to_2[0])
                        to_res = int(from_to_2[1])
                    else:
                        from_res = int(from_to[1])
                        to_res = int(from_to[1])
                else:
                    from_res = from_to[0]
                    to_res = from_to[0]
        ...
        if line.startswith("/note"):
            ...
            description = (
                old_desc + " (" + line.lstrip('/evidence="').rstrip().rstrip('"')
            )

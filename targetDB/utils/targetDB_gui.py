...
class ScrolledFrame(tk.Frame):
    def __init__(self, parent, vertical=True, horizontal=False):
        ...
        self._vertical_bar = tk.Scrollbar(
        ...
        self._horizontal_bar = tk.Scrollbar(
...
class get_mpo_coeff_gui:
    def __init__(self):
        ...
        while sum(self.coeff.values()) == 0:
            showerror(
                "Error",
                "Please set at least one coefficient to a value different from 0",
            )
            self.master = tk.Toplevel()
            self.create_window()
    ...
    def create_window(self):
        ...
        tk.Label(
            self.master,
            height=2,
            text="Please input the desired weigth for the MPO score component",
            font="bold",
        ).grid(row=0, columnspan=8)
        for i in range(len(headers)):
            tk.Label(
                self.master, height=2, text=headers[i], font="bold", bg=colors[i]
            ).grid(row=2, column=i, sticky=tk.W + tk.E)
        tk.Button(
            self.master,
            text="Validate",
            ...
            fg="red",
            bg="white",
        ).grid(columnspan=8, row=1)
        self.structural_info = tk.Scale(
            self.master,
            from_=200,
            to=-200,
            orient=tk.VERTICAL,
    ...
    def get_values(self):
        self.coeff = {
            "sbio": self.structural_info.get() / 100,
            "sdrug": self.structural_drug.get() / 100,
            "chem": self.chemistry.get() / 100,
            "bio": self.biology.get() / 100,
            "dise": self.disease.get() / 100,
            "gen": self.genetic.get() / 100,
            "info": self.information.get() / 100,
            "safe": self.safety.get() / 100,
        }
        self.over = True
...
class config_gui:
    ...
    def config_panel(self, new=False):
        ...
        self.w_header = tk.Label(
            self.config_window,
            text="CONFIGURATION",
            bg="#bbf7b4",
            font="Helvetica 16 bold",
        )
        self.w_section_database = tk.Label(
            self.config_window, text="Databases", relief="raised", bg="#e6e6e6"
        )
        self.w_section_outputs = tk.Label(
            self.config_window, text="Outputs", relief="raised", bg="#e6e6e6"
        )
        self.w_section_pubmed = tk.Label(
            self.config_window, text="Pubmed", relief="raised", bg="#e6e6e6"
        )
        ...
        self.w_pubmed_msg = tk.Message(
            self.config_window,
            textvariable=self.message_pubmed,
            justify="center",
            width=500,
        )
        self.targetdb_path = tk.StringVar()
        self.w_targetdb = tk.Entry(
            self.config_window, textvariable=self.targetdb_path, width=100
        )
        self.list_path = tk.StringVar()
        self.w_list = tk.Entry(
            self.config_window, textvariable=self.list_path, width=100
        )
        self.single_path = tk.StringVar()
        self.w_single = tk.Entry(
            self.config_window, textvariable=self.single_path, width=100
        )
        self.pubmed_email = tk.StringVar()
        self.w_pubmed_email = tk.Entry(
            self.config_window, textvariable=self.pubmed_email, width=100
        )
        ...
        tk.Message(
            self.config_window,
            text="""Please browse to a valid targetDB database file:""",
            width=500,
        ).grid(row=2, column=0)
        ...
        tk.Message(
            self.config_window,
            text="""Please select a folder in which to save LISTS outputs:""",
            width=500,
        ).grid(row=5, column=0)
        ...
        tk.Button(
            ...
            text="Browse...",
            bg="#a0a0a0",
            command=self.select_folder_list,
        ).grid(row=5, column=4)

        tk.Message(
            self.config_window,
            text="""Please select a folder in which to save SINGLE outputs:""",
            width=500,
        ).grid(row=7, column=0)
        ...
        tk.Button(
            ...
            text="Browse...",
            bg="#a0a0a0",
            command=self.select_folder_single,
        ).grid(row=7, column=4)
    ...
    def select_targetdb(self):
        targetDB_path = Path(
            askopenfilename(
                title="Select TargetDB sqlite database",
...
class targetDB_gui:
    def __init__(self):
        ...
        self.mode_label = tk.Label(
            self.window, text="Mode", relief="raised", bg="#e6e6e6"
        )
        self.mode_single = tk.Radiobutton(
            self.window, text="Single mode", variable=self.mode, value="single"
        )
        self.mode_list = tk.Radiobutton(
            self.window, text="List mode", variable=self.mode, value="list"
        )
        self.mode_spider = tk.Radiobutton(
            self.window, text="Spider plot only", variable=self.mode, value="spider"
        )

        self.target_input_label = tk.Label(
            self.window, text="Targets list", relief="raised", bg="#e6e6e6"
        )
        ...
        self.target_message = tk.Message(
            self.window, textvariable=self.message_target, justify="center"
        )
        ...
        self.go_button.grid(
            row=8, column=1, columnspan=2, sticky="ew", pady=10, padx=10
        )
    ...
    def launch(self):
        ...
        target_list = list(filter(None, target_list))
        self.gene_df, self.not_found = g2id.gene_to_id(
            target_list, targetDB_path=self.targetDB_path
        )
        message = ""
        if mode == "list":
        ...
        if mode == "spider":
            ...
            for gene_name in self.gene_df.index:
                ...
                b = Button(
                    ...
                    bg="red",
                    fg="white",
                    command=lambda x=gene_idx: self.save_fig(n=x),
                )
                b.pack()
    ...
    def make_spider_plot_v3(
        self, data, data_qual, labels, druggability_val=0, target_name=""
    ):

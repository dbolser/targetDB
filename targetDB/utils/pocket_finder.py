...
def get_object(p_dict, pdb_name, out_pockets_dir_path, pdb_info, domain):
    ...
    for key, values in p_dict.items():
        pockets_objects[key] = Pockets(
            values, pdb_name, key, out_pockets_dir_path, pdb_info, domain
        )
...
def get_pockets(
    ...
    if os.name == "nt":
        return {
            "pockets": pock,
            "pockets_chain": pock_chain,
            "pockets_domain": pock_domain,
        }
    ...
    for f in files:
                    ...
                    if not chain_to_keep:
                        for c in pdb_info.loc[pdb_code]["Chain"]:
                            if c in chains:
                                ...
                                pdb_strip_path = pocket_path.joinpath(
                                    pdb_code + "_BIO" + biomolecule + ".pdb"
                                )
                                out_path = pocket_path.joinpath(
                                    pdb_code + "_BIO" + biomolecule + "_out"
                                )
                                out_file_path = out_path.joinpath(
                                    pdb_code + "_BIO" + biomolecule + "_info.txt"
                                )
                                out_pockets_dir_path = out_path.joinpath("pockets")
                                break
        ...
        if out_path.is_dir():
            if out_file_path.is_file():
                if out_pockets_dir_path.is_dir():
                    ...
                    if p_out != 0:
                        ...
                        results[pdb_code] = get_object(
                            pockets, pdb_code, str(out_pockets_dir_path), info, domain
                        )
                        continue
                    else:
                        fpocket_launcher(
            ...
            fpocket_launcher(
                pdb_strip_path, pdb_file, sphere_size_min=sphere_size, verbose=verbose
            )
        if out_path.is_dir():
            if out_file_path.is_file():
                if out_pockets_dir_path.is_dir():
                    ...
                    if p_out != 0:
                        ...
                        print(
                            "[ERROR]: No output file detected (problem with PDB file or no pocket found): "
                            + pdb_file
                        )
        ...
        for pdb_code, pockets in results.items():
            for p in pockets.keys():
                ...
                pock.loc[pocket_id] = {
                    ...
                    "Pocket_number": pockets[p].pocket_number,
                    "Pocket_id": pocket_id,
                    "Score": pockets[p].score,
                    "DrugScore": pockets[p].druggability_score,
                    "apolar_sasa": pockets[p].apolar_sasa,
                    "polar_sasa": pockets[p].polar_sasa,
                    "total_sasa": pockets[p].total_sasa,
                    "volume": pockets[p].volume,
                    "blast": blast,
                    "druggable": pockets[p].druggable,
                }

                if pockets[p].chain_coverage:
                    for c in pockets[p].chain_coverage.keys():
                        pock_chain.loc[pocket_id + "_" + c] = {
                            "Pocket_id": pocket_id,
                            "Chain_id": pdb_code + "_" + c,
                            "List_of_contacts": pockets[p].chain_coverage[c],
                        }
                if pockets[p].part_of_domain:
                    for d in pockets[p].part_of_domain:
                        pock_domain.loc[pocket_id + "_" + d["domain_id"]] = {
                            "Pocket_id": pocket_id,
    ...
    for k in dict_pdbs.keys():
        for p in dict_pdbs[k].keys():
            ...
            for d in dict_pdbs[k][p].part_of_domain:
                if d["coverage"] > 5.0:
                    in_list = True
                    rank_list[d["domain"]].append(
                        (k, p, float(dict_pdbs[k][p].param[rank_by]))
                    )
            if not in_list:
...
class Pockets:
    ...
    def __init__(
        self, values, pdb_name, pnumber, out_pockets_dir_path, pdb_info, domain
    ):
        ...
        if not pdb_info.empty and not domain.empty:
            ...
            self.part_of_domain = []
        if (
            float(self.param["druggability_score"]) >= 0.5
            and float(self.param["volume"]) >= 250.0
        ):
            self.druggable = "TRUE"
        ...
        for k in self.chain_coverage.keys():
            if k in pdb_info["Chain"]:
                for res_number in self.chain_coverage[k]:
                    for d in domain.index:
                        if (
                            int(domain.loc[d]["Start"])
                            <= int(res_number)
                            <= int(domain.loc[d]["Stop"])
                        ):
                            ...
                            self.part_of_domain.append(
                                {
                                    "domain": key,
                                    "coverage": round(
                                        (domain_coverage[key] / domain.loc[d]["length"])
                                        * 100,
                                        0,
                                    ),
            ...
            try:
                if pdb_info["length"] == 0:
                    ...
                    self.part_of_domain.append(
                        {
                            "domain": "other",
                            "coverage": round((count / pdb_info["length"]) * 100, 0),
                            "domain_id": "other",
                        }
                    )
            except KeyError:

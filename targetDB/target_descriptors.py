...
class target_scores:
    def __init__(self, data, mode="list"):
        ...
        self.score_components = self.data.Target_id.reset_index().drop(
            ["index"], axis=1
        )
    ...
    def get_mpo_score(self):
        if self.mode == "list":
            ...
            self.scores_mpo["mpo_score"] = (
                self.scores_mpo.safety_score * coeff_df["safe"]
                + self.scores_mpo.information_score * coeff_df["info"]
                + self.scores_mpo.genetic_score * coeff_df["gen"]
                + self.scores_mpo.disease_score * coeff_df["dise"]
                + self.scores_mpo.biology_score * coeff_df["bio"]
                + self.scores_mpo.chemistry_score * coeff_df["chem"]
                + self.scores_mpo.structural_drug_score * coeff_df["sdrug"]
                + self.scores_mpo.structure_info_score * coeff_df["sbio"]
            ) / sum([abs(v) for v in coeff_df.values()])
    ...
    def get_structure_info_score(self):
        ...
        # ==============================================================================================#
        structural_info_cols = [
            "Target_id",
            "PDB_total_count",
            "%_sequence_covered",
            "%_domain_covered",
            "PDB_blast_close_count",
            "PDB_blast_max_similarity",
        ]
        structural_info_df = self.data[structural_info_cols].copy()
        structural_info_df["pdb_count_score"] = np.where(
            structural_info_df.PDB_total_count >= 3, 1, 0
        )
        structural_info_df["pdb_alt_count_score"] = np.where(
            (
                (
                    structural_info_df.PDB_blast_close_count
                    * (structural_info_df.PDB_blast_max_similarity / 100)
                )
                / 2
            )
            >= 3,
        ...
        self.score_components = self.score_components.merge(
            struct_info_components, on="Target_id", how="left"
        )

    def get_structure_druggability_score(self):
        structural_drug_cols = [
            "Target_id",
            "domain_tractable",
            "domain_druggable",
            "mean_druggability_score",
            "mean_alt_druggability_score",
            "mean_alt_similarity",
        ]
        structural_drug_df = self.data[structural_drug_cols].copy()
        structural_drug_df["domain_drug_score"] = structural_drug_df[
            ["domain_tractable", "domain_druggable"]
        ].mean(axis=1)
        structural_drug_df["alt_drug_score"] = (
            structural_drug_df.mean_alt_druggability_score
            * (structural_drug_df.mean_alt_similarity / 100)
        )
        structural_drug_df["structural_drug_score"] = structural_drug_df[
            ["mean_druggability_score", "domain_drug_score", "alt_drug_score"]
        ].mean(axis=1)
        self.scores_quality = self.scores_quality.merge(
            structural_drug_df[["Target_id", "structural_drug_score"]],
            on="Target_id",
            how="left",
        )
    ...
    def get_chem_score(self):
        chemistry_cols = [
            "Target_id",
            "BindingDB_potent_count",
            "BindingDB_potent_phase2_count",
            "ChEMBL_bioactives_potent_count",
            "ChEMBL_bioactives_moderate_selectivity_count",
            "ChEMBL_bioactives_good_selectivity_count",
            "ChEMBL_bioactives_great_selectivity_count",
            "commercial_potent_total",
        ]
        chemistry_df = self.data[chemistry_cols].copy()
        chemistry_df["bindingDB_potent"] = norm_min_max(
            np.log(pd.to_numeric(chemistry_df.BindingDB_potent_count, errors="coerce")),
            df_max=8,
        )
        chemistry_df["bindingDB_potent_log"] = norm_min_median(
            np.log(pd.to_numeric(chemistry_df.BindingDB_potent_count, errors="coerce")),
            median=3.13,
        )
        chemistry_df["bindingDB_phase2"] = norm_min_max(
            np.log(
                pd.to_numeric(
                    chemistry_df.BindingDB_potent_phase2_count, errors="coerce"
                )
            ),
            df_max=4.7,
        )
        chemistry_df["chembl_potent"] = norm_min_max(
            np.log(
                pd.to_numeric(
                    chemistry_df.ChEMBL_bioactives_potent_count, errors="coerce"
                )
            ),
            df_max=8,
        )
        chemistry_df["chembl_potent_log"] = norm_min_median(
            np.log(
                pd.to_numeric(
                    chemistry_df.ChEMBL_bioactives_potent_count, errors="coerce"
                )
        ...
        chemistry_df["chembl_selective_M"] = norm_min_max(
            np.log(
                pd.to_numeric(
                    chemistry_df.ChEMBL_bioactives_moderate_selectivity_count,
                    errors="coerce",
                )
            ),
            df_max=7,
        )
        chemistry_df["chembl_selective_G"] = norm_min_max(
            np.log(
                pd.to_numeric(
                    chemistry_df.ChEMBL_bioactives_good_selectivity_count,
                    errors="coerce",
                )
            ),
            df_max=7,
        ...
        chemistry_df["chembl_selective_E"] = norm_min_max(
            np.log(
                pd.to_numeric(
                    chemistry_df.ChEMBL_bioactives_great_selectivity_count,
                    errors="coerce",
                )
            ),
            df_max=7,
        )
        chemistry_df["commercial_potent"] = norm_min_max(
            np.log(
                pd.to_numeric(chemistry_df.commercial_potent_total, errors="coerce")
            ),
            df_max=5,
        )

        chemistry_df["chemistry_score"] = chemistry_df[
            ["chembl_potent_log", "bindingDB_potent_log"]
        ].mean(axis=1)
        chemistry_df["chemistry_qual_score"] = np.where(
            chemistry_df.BindingDB_potent_phase2_count > 0,
            1,
            np.where(
                ...
                np.where(
                    ...
                    np.where(
                        ...
                        np.where(
                            ...
                            0.3,
                            np.where(chemistry_df.BindingDB_potent_count > 0, 0.3, 0),
                        ),
                    ),
                ),
            ),
        ...
        self.scores = self.scores.merge(
            chemistry_df[["Target_id", "chemistry_score"]], on="Target_id", how="left"
        )
        self.scores_quality = self.scores_quality.merge(
            chemistry_df[["Target_id", "chemistry_qual_score"]],
            on="Target_id",
            how="left",
        )
    ...
    def get_bio_score(self):
        ...
        biology_df["bio_PScore"] = (
            np.where(biology_df.kegg_count > 0, 1, 0)
            + np.where(biology_df.reactome_count > 0, 1, 0)
        ) / 2
        ...
        self.scores = self.scores.merge(
            biology_df[["Target_id", "biology_score"]], on="Target_id", how="left"
        )
        self.score_components = self.score_components.merge(
            biology_df[
                [
    ...
    def get_disease_score(self):
        disease_cols = [
            "Target_id",
            "disease_count_uniprot",
            "disease_count_tcrd",
            "OT_number_of_disease_areas",
            "OT_max_association_score",
        ]
        disease_df = self.data[disease_cols].copy()
        disease_df["dis_AScore"] = (
            np.where(disease_df.OT_number_of_disease_areas > 0, 1, 0)
            + np.where(disease_df.OT_number_of_disease_areas > 1, 1, 0)
        ) / 2
        ...
        disease_df["disease_score"] = (
            disease_df["OT_max_association_score"] * 2
            + disease_df.dis_AScore
            + disease_df.dis_UScore
            + disease_df.dis_TScore
        ) / 5
        ...
        self.scores = self.scores.merge(
            disease_df[["Target_id", "disease_score"]], on="Target_id", how="left"
        )
        self.score_components = self.score_components.merge(
            disease_score_component, on="Target_id", how="left"
        )
    ...
    def get_genetic_score(self):
        col_genetic = [
            ...
            "gwas_count",
            "gwas_sig",
            "OT_MAX_VAL_genetic_association",
            "OT_NUM_MAX_genetic_association",
            "OT_TOP10_avg_score",
        ]
        genetic_df = self.data[col_genetic].copy()
        genetic_df["genetic_NORM"] = norm_min_max(
            np.log2(
                pd.to_numeric(
                    genetic_df.OT_NUM_MAX_genetic_association.replace(0, 0.0001),
                    errors="coerce",
                )
            ),
            df_max=5,
        )
        genetic_df["gen_GScore"] = norm_min_max(
            np.log10(pd.to_numeric(genetic_df.gwas_count * 10, errors="coerce")),
            df_min=0,
            df_max=2,
        )
        genetic_df["gen_GQualScore"] = genetic_df.gwas_sig / genetic_df.gwas_count
        genetic_df["gen_AScore"] = (
            genetic_df.OT_MAX_VAL_genetic_association * genetic_df.genetic_NORM
        )
        genetic_df["gen_AQualScore"] = genetic_df.OT_TOP10_avg_score
        genetic_df["genetic_score"] = genetic_df[["gen_GScore", "gen_AScore"]].mean(
            axis=1
        )
        genetic_df["genetic_score_qual"] = genetic_df[
            ["gen_GQualScore", "gen_AQualScore"]
        ].mean(axis=1)
        ...
        self.scores = self.scores.merge(
            genetic_df[["Target_id", "genetic_score"]], on="Target_id", how="left"
        )
        self.scores_quality = self.scores_quality.merge(
            genetic_df[["Target_id", "genetic_score_qual"]], on="Target_id", how="left"
        )

    def get_info_score(self):
        col_info = ["Target_id", "JensenLab PubMed Score"]
        info_df = self.data[col_info].copy()
        info_df["information_score"] = norm_min_max(
            np.log(
                pd.to_numeric(
                    info_df["JensenLab PubMed Score"].replace(0, 0.00001),
                    errors="coerce",
                )
            ),
            df_max=12,
        ...
        self.scores = self.scores.merge(
            info_df[["Target_id", "information_score"]], on="Target_id", how="left"
        )
        self.score_components = self.score_components.merge(
            info_df[["Target_id", "information_score"]], on="Target_id", how="left"
        )
    ...
    def get_safety_score(self):
        ...
        safety_df["safe_GScore"] = (
            (
                safety_df.phenotypes_homozygotes_lethal_count.fillna(0) * 2
                + safety_df.phenotypes_heterozygotes_lethal_count.fillna(0)
                - safety_df.phenotypes_heterozygotes_normal_count.fillna(0)
                - 2 * safety_df.phenotypes_homozygotes_normal_count.fillna(0)
            )
            / safety_df.number_of_genotypes
        ).clip(0, 1)

        safety_df["safe_EScore"] = np.where(
            safety_df[["Heart_alert", "Liver_alert", "Kidney_alert"]].any(axis=1),
            1,
            np.where(
                safety_df[["Heart_alert", "Liver_alert", "Kidney_alert"]]
                .isna()
                .all(axis=1),
                np.nan,
                0,
        ...
        safety_df["safety_qual"] = safety_df[["safe_GScore", "safe_EScore"]].mean(
            axis=1
        )
        safety_df["n_genotypes"] = norm_min_max(
            np.log(pd.to_numeric(safety_df.number_of_genotypes, errors="coerce")),
            df_max=6,
        )
        safety_df["safety_score"] = (
            safety_df.n_genotypes + np.where(self.data.Brain.isna(), 0, 0.3)
        ).clip(0, 1)
        ...
        self.score_components = self.score_components.merge(
            safety_score_component, on="Target_id", how="left"
        )
        self.scores = self.scores.merge(
            safety_df[["Target_id", "safety_score"]], on="Target_id", how="left"
        )
        self.scores_quality = self.scores_quality.merge(
            safety_df[["Target_id", "safety_qual"]], on="Target_id", how="left"
        )
...
def get_descriptors_list(target_id, targetdb=None):
    ...
    list_queries = {
        "gen_info": """SELECT * FROM Targets WHERE Target_id in ('%s')""" % target_id,
        "disease": """SELECT Target_id,disease_name,disease_id FROM disease WHERE Target_id in ('%s')"""
        % target_id,
        "reactome": """SELECT pathway_name,Target_id FROM pathways WHERE pathway_dataset='Reactome pathways data set' AND Target_id in ('%s')"""
        % target_id,
        "kegg": """SELECT Target_id,pathway_name FROM pathways WHERE pathway_dataset='KEGG pathways data set' AND Target_id in ('%s')"""
        % target_id,
        "gwas": """SELECT
      Target_id,
      phenotype,
    ...
    WHERE I.Target_id in ('%s') ORDER BY I.Canonical DESC"""
        % target_id,
    ...
    WHERE M.mod_type = 'VAR' AND M.Target_id in ('%s')"""
        % target_id,
      ...
      M.comment AS comments
    FROM modifications M
    WHERE M.mod_type = 'MUTAGEN' AND M.Target_id in ('%s')"""
        % target_id,
        "domains": """SELECT
      Target_id,
      Domain_name,
    ...
    GROUP BY C.Target_id,C.PDB_code,P.Technique,P.Resolution,C.n_residues,C.start_stop"""
        % target_id,
        "pockets": """SELECT
      F.Target_id,
      F.PDB_code,
    ...
    WHERE target_id in ('%s')"""
        ...
        "domain_drugE": """SELECT Target_id,
                    ...
                    GROUP BY Target_id"""
        % target_id,
        "opentargets": """SELECT * FROM opentarget_association WHERE target_id in ('%s')"""
        % target_id,
    }

    results = {
        qname: pd.read_sql(query, con=connector_targetDB)
        for qname, query in list_queries.items()
    }
    # ================================================================================================================#
    # ========================================= GENERAL INFO =========================================================#
    # ================================================================================================================#

    data = results["gen_info"].drop(
        [
            "Species",
            "species_id",
            "Sequence",
    ...
    data = data.merge(
        results["kegg"]
        .groupby("Target_id")["pathway_name"]
        .apply("\n".join)
        .reset_index()
        .rename(columns={"pathway_name": "kegg_list"}),
        on="Target_id",
        how="left",
    )
    ...
    data = data.merge(
        ...
        .count()
        .reset_index()
        .rename(columns={"pathway_name": "kegg_count"}),
        on="Target_id",
        how="left",
    )
    ...
    data = data.merge(
        ...
        .apply("\n".join)
        .reset_index()
        .rename(columns={"pathway_name": "reactome_list"}),
        on="Target_id",
        how="left",
    )
    ...
    data = data.merge(
        ...
        .count()
        .reset_index()
        .rename(columns={"pathway_name": "reactome_count"}),
        on="Target_id",
        how="left",
    )
    ...
    data = data.merge(
        ...
        .count()
        .reset_index()
        .rename(columns={"disease_id": "disease_count_uniprot"}),
        on="Target_id",
        how="left",
    )
    ...
    if OT_diseases.empty:
        OT = pd.DataFrame(
            columns=[
                "Target_id",
                "OT_number_of_associations",
                "OT_list_max_diseases",
                "OT_TOP10_diseases",
                "OT_max_association_score",
                "OT_%_genetic_association",
                "OT_%_known_drug",
                "OT_%_litterature_mining",
                "OT_TOP10_avg_score",
                "OT_%_animal_model",
                "OT_%_affected_pathway",
                "OT_%_rna_expression",
                "OT_%_somatic_mutation",
                "OT_MAX_VAL_genetic_association",
                "OT_NUM_MAX_genetic_association",
                "OT_MAX_VAL_known_drug",
                "OT_NUM_MAX_known_drug",
                "OT_MAX_VAL_litterature_mining",
                "OT_NUM_MAX_litterature_mining",
                "OT_MAX_VAL_animal_model",
                "OT_NUM_MAX_animal_model",
                "OT_MAX_VAL_affected_pathway",
                "OT_NUM_MAX_affected_pathway",
                "OT_MAX_VAL_rna_expression",
                "OT_NUM_MAX_rna_expression",
                "OT_MAX_VAL_somatic_mutation",
                "OT_NUM_MAX_somatic_mutation",
            ]
        )
    else:
        OT_n_associations = (
            OT_diseases.groupby("target_id")["disease_area"]
            .count()
            .reset_index()
            .rename(
                columns={
                    "target_id": "Target_id",
                    "disease_area": "OT_number_of_associations",
                }
            )
        )
        OT_disease_max = OT_diseases.loc[
            OT_diseases.groupby("target_id").idxmax().overall_score
        ][["target_id", "overall_score"]].rename(
            columns={
                "target_id": "Target_id",
                "overall_score": "OT_max_association_score",
            }
        )
        OT_max_disease_list = (
            OT_diseases.groupby("target_id")
            .apply(lambda x: x[x.overall_score == x.overall_score.max()])
            .reset_index(drop=True)
            .groupby("target_id")["disease_name"]
            .apply(",".join)
            .reset_index()
            .rename(
                columns={
                    "target_id": "Target_id",
                    "disease_name": "OT_list_max_diseases",
                }
            )
        )
        OT_disease_association_type_proportion = (
            OT_diseases.groupby("target_id")
            .apply(lambda x: x[x != 0].count() / x[x != 0]["target_id"].count())
            .drop(
                ["target_id", "disease_area", "disease_name", "overall_score"], axis=1
            )
            .reset_index()
            .rename(
                columns={
                    "target_id": "Target_id",
                    "genetic_association": "OT_%_genetic_association",
                    "known_drug": "OT_%_known_drug",
                    "litterature_mining": "OT_%_litterature_mining",
                    "animal_model": "OT_%_animal_model",
                    "affected_pathway": "OT_%_affected_pathway",
                    "rna_expression": "OT_%_rna_expression",
                    "somatic_mutation": "OT_%_somatic_mutation",
                }
            )
            .round(2)
        )
        OT_diseases_associations_max_count = (
            OT_diseases.drop(["disease_area", "disease_name", "overall_score"], axis=1)
            .groupby("target_id")
            .apply(lambda x: max_range(x, dx=0.1))
            .reset_index()
            .rename(
                columns={
                    "target_id": "Target_id",
                    "genetic_association": "OT_NUM_MAX_genetic_association",
                    "known_drug": "OT_NUM_MAX_known_drug",
                    "litterature_mining": "OT_NUM_MAX_litterature_mining",
                    "animal_model": "OT_NUM_MAX_animal_model",
                    "affected_pathway": "OT_NUM_MAX_affected_pathway",
                    "rna_expression": "OT_NUM_MAX_rna_expression",
                    "somatic_mutation": "OT_NUM_MAX_somatic_mutation",
                }
            )
        )
        OT_diseases_associations_max_val = (
            OT_diseases.groupby("target_id")
            .max()
            .drop(["disease_area", "disease_name", "overall_score"], axis=1)
            .reset_index()
            .rename(
                columns={
                    "target_id": "Target_id",
                    "genetic_association": "OT_MAX_VAL_genetic_association",
                    "known_drug": "OT_MAX_VAL_known_drug",
                    "litterature_mining": "OT_MAX_VAL_litterature_mining",
                    "animal_model": "OT_MAX_VAL_animal_model",
                    "affected_pathway": "OT_MAX_VAL_affected_pathway",
                    "rna_expression": "OT_MAX_VAL_rna_expression",
                    "somatic_mutation": "OT_MAX_VAL_somatic_mutation",
                }
            )
        )
        OT_top10 = OT_diseases.groupby("target_id").apply(
            lambda x: x.loc[x.overall_score.nlargest(10).index]
        )
        OT_top10_qual = OT_diseases.groupby("target_id").apply(
            lambda x: x.loc[x.overall_score.nlargest(10).index]
        )
        OT_top10_qual = (
            OT_top10_qual["overall_score"]
            .reset_index()
            .groupby("target_id")["overall_score"]
            .mean()
            .reset_index()
            .rename(
                columns={
                    "target_id": "Target_id",
                    "overall_score": "OT_TOP10_avg_score",
                }
            )
        )
        OT_top10 = OT_top10[OT_top10.overall_score >= 0.1]
        OT_top10 = (
            OT_top10["disease_name"]
            .reset_index()
            .groupby("target_id")["disease_name"]
            .apply(",".join)
            .reset_index()
            .rename(
                columns={"target_id": "Target_id", "disease_name": "OT_TOP10_diseases"}
            )
        )
        ...
        OT = OT.merge(
            OT_disease_association_type_proportion, on="Target_id", how="left"
        )
    ...
    OT_disease_areas = results["opentargets"][
        (results["opentargets"]["disease_area"] == "")
    ]
    ...
    if OT_disease_areas.empty:
        OT = OT.merge(
            pd.DataFrame(
                columns=[
                    "Target_id",
                    "OT_number_of_disease_areas",
                    "OT_list_max_disease_area",
                    "OT_max_association_diseaseArea_score",
                ]
            ),
            on="Target_id",
            how="left",
        ...
        OT_n_disease_areas = (
            OT_disease_areas.groupby("target_id")["disease_area"]
            .count()
            .reset_index()
            .rename(
                columns={
                    "target_id": "Target_id",
                    "disease_area": "OT_number_of_disease_areas",
                }
            )
        )
        OT_disease_area_max = OT_disease_areas.loc[
            OT_disease_areas.groupby("target_id").idxmax().overall_score
        ][["target_id", "overall_score"]].rename(
            columns={
                "target_id": "Target_id",
                "overall_score": "OT_max_association_diseaseArea_score",
            }
        )
        OT_max_disease_area_list = (
            OT_disease_areas.groupby("target_id")
            .apply(lambda x: x[x.overall_score == x.overall_score.max()])
            .reset_index(drop=True)
            .groupby("target_id")["disease_name"]
            .apply(",".join)
            ...
            .rename(
                columns={
                    "target_id": "Target_id",
                    "disease_name": "OT_list_max_disease_area",
                }
            )
        )
    ...
    if not results["pockets"].empty:
        ...
        data = data.merge(
            results["pockets"]
            .groupby("Target_id")["druggability_score"]
            .std()
            .reset_index()
            .rename(columns={"druggability_score": "stddev_druggability_score"})
            .round(2),
            on="Target_id",
            how="left",
        )
        data = data.merge(
            results["pockets"]
            .groupby("Target_id")["PDB_code"]
            .nunique()
            .reset_index()
            .rename(columns={"PDB_code": "pdb_with_druggable_pocket"}),
            on="Target_id",
            how="left",
        )
        data = data.merge(
            ...
            .count()
            .reset_index()
            .rename(columns={"PDB_code": "druggable_pockets_total"}),
            on="Target_id",
            how="left",
        )
    else:
        pocket = pd.DataFrame(
            columns=[
                "Target_id",
                "mean_druggability_score",
                "stddev_druggability_score",
                "mean_area",
                "mean_volume",
                "mean_fraction_apolar",
                "mean_pocket_score",
                "pdb_with_druggable_pocket",
                "druggable_pockets_total",
            ]
        )
    ...
    if not results["alt_pockets"].empty:
        ...
        data = data.merge(
            results["alt_pockets"]
            .groupby("Target_id")["alt_druggability_score"]
            .std()
            .reset_index()
            .rename(columns={"alt_druggability_score": "alt_stddev_druggability_score"})
            .round(2),
            on="Target_id",
            how="left",
        )
        data = data.merge(
            results["alt_pockets"]
            .groupby("Target_id")["alt_similarity"]
            .max()
            .reset_index()
            .rename(columns={"alt_similarity": "max_alt_similarity"}),
            on="Target_id",
            how="left",
        )
        ...
        data = data.merge(
            ...
            .nunique()
            .reset_index()
            .rename(columns={"PDB_code": "alt_pdb_with_druggable_pocket"}),
            on="Target_id",
            how="left",
        )
        data = data.merge(
            ...
            .count()
            .reset_index()
            .rename(columns={"PDB_code": "alt_druggable_pockets_total"}),
            on="Target_id",
            how="left",
        )
    else:
        alt_pocket = pd.DataFrame(
            columns=[
                "Target_id",
                "mean_alt_druggability_score",
                "alt_stddev_druggability_score",
                "mean_alt_area",
                "mean_alt_volume",
                "mean_alt_fraction_apolar",
                "mean_alt_pocket_score",
                "mean_alt_similarity",
                "max_alt_similarity",
                "alt_pdb_with_druggable_pocket",
                "alt_druggable_pockets_total",
            ]
        )
    ...
    results["bindingDB"]["IC50(nM)"] = pd.to_numeric(
        results["bindingDB"]["IC50(nM)"], errors="coerce"
    )
    results["bindingDB"]["EC50(nM)"] = pd.to_numeric(
        results["bindingDB"]["EC50(nM)"], errors="coerce"
    )
    results["bindingDB"]["Ki(nM)"] = pd.to_numeric(
        results["bindingDB"]["Ki(nM)"], errors="coerce"
    )
    results["bindingDB"]["Kd(nM)"] = pd.to_numeric(
        results["bindingDB"]["Kd(nM)"], errors="coerce"
    )

    potent = results["bindingDB"][
        (
            (results["bindingDB"][["IC50(nM)", "EC50(nM)", "Kd(nM)", "Ki(nM)"]] <= 100)
            & (
                ~results["bindingDB"][
                    ["IC50(nM)", "EC50(nM)", "Kd(nM)", "Ki(nM)"]
                ].isnull()
            )
        ).any(axis=1)
    ]
    ...
    data = data.merge(
        results["bindingDB"]
        .groupby("target_id")["ligand_name"]
        .count()
        .reset_index()
        .rename(columns={"target_id": "Target_id", "ligand_name": "BindingDB_count"}),
        on="Target_id",
        how="left",
    )
    data = data.merge(
        potent.groupby("target_id")["ligand_name"]
        .count()
        .reset_index()
        .rename(
            columns={"target_id": "Target_id", "ligand_name": "BindingDB_potent_count"}
        ),
        on="Target_id",
        how="left",
    )
    data = data.merge(
        potent_max_phase.groupby("target_id")["ligand_name"]
        .count()
        .reset_index()
        .rename(
            columns={
                "target_id": "Target_id",
                "ligand_name": "BindingDB_potent_phase2_count",
            }
        ),
        on="Target_id",
        how="left",
    ...
    if not results["tissue_expression"].empty:
        cell_values = (
            results["tissue_expression"].groupby(["Target_id", "organ", "tissue"]).max()
        )
        cell_values.value = cell_values.value.replace(0, np.nan)
        tissue_grouped = (
            cell_values.groupby(["Target_id", "organ"])
            .mean(numeric_only=True)
            .round(1)
            .reset_index()
            .fillna(0)
        )
        tissue = tissue_grouped.pivot(
            index="Target_id", columns="organ", values="value"
        ).reset_index()
        tissue_avg = (
            tissue_grouped.groupby("Target_id")["value"]
            .agg(["mean", "std"])
            .rename(columns={"mean": "EXP_LVL_AVG", "std": "EXP_LVL_STDDEV"})
            .reset_index()
        )
        ...
        for tid in tissue_avg.index:
            avg_exp_1stddev = (
                tissue_avg.loc[tid].EXP_LVL_AVG + tissue_avg.loc[tid].EXP_LVL_STDDEV
            )
            try:
                if (
                    cell_values.loc[(tid, "Kidney", "kidney")].value > avg_exp_1stddev
                    or cell_values.loc[(tid, "Kidney", "kidney")].value == 3
                ):
                    tissue_avg.loc[tid, ["Kidney_alert"]] = True
                    tissue_avg.loc[tid, ["Kidney_value"]] = cell_values.loc[
                        (tid, "Kidney", "kidney")
                    ].value
            except KeyError:
                pass
            try:
                if (
                    cell_values.loc[(tid, "Liver_gallbladder", "liver")].value
                    > avg_exp_1stddev
                    or cell_values.loc[(tid, "Liver_gallbladder", "liver")].value == 3
                ):
                    tissue_avg.loc[tid, ["Liver_alert"]] = True
                    tissue_avg.loc[tid, ["Liver_value"]] = cell_values.loc[
                        (tid, "Liver_gallbladder", "liver")
                    ].value
            except KeyError:
                pass
            try:
                if (
                    cell_values.loc[(tid, "Muscle_tissue", "heart muscle")].value
                    > avg_exp_1stddev
                    or cell_values.loc[(tid, "Muscle_tissue", "heart muscle")].value
                    == 3
                ):
                    tissue_avg.loc[tid, ["Heart_alert"]] = True
                    tissue_avg.loc[tid, ["Heart_value"]] = cell_values.loc[
                        (tid, "Muscle_tissue", "heart muscle")
                    ].value
            except KeyError:
                pass
        tissue_avg.reset_index(drop=True, inplace=True)
        tissue_max = tissue_grouped.loc[
            tissue_grouped.groupby("Target_id").idxmax()["value"]
        ].rename(
        ...
        tissue_max = pd.DataFrame(
            columns=["tissue_max_expression", "expression_max_tissue", "Target_id"]
        )
    data = data.merge(tissue, on="Target_id", how="left")
    data = data.merge(
        results["selectivity"]
        .round(2)
        .rename(columns={"Selectivity_entropy": "Expression_Selectivity"}),
        on="Target_id",
        how="left",
    )
    ...
    data = data.merge(
        results["var"]
        .groupby("Target_id")["start"]
        .count()
        .reset_index()
        .rename(columns={"start": "variants_count"}),
        on="Target_id",
        how="left",
    )
    data = data.merge(
        ...
        .count()
        .reset_index()
        .rename(columns={"start": "mutants_count"}),
        on="Target_id",
        how="left",
    )
    data = data.merge(
        results["gwas"]
        .groupby("Target_id")["phenotype"]
        .count()
        .reset_index()
        .rename(columns={"phenotype": "gwas_count"}),
        on="Target_id",
        how="left",
    )
    data = data.merge(
        results["gwas"][results["gwas"]["p_value"] <= 0.000000005]
        .groupby("Target_id")["phenotype"]
        .count()
        .reset_index()
        .rename(columns={"phenotype": "gwas_sig"}),
        on="Target_id",
        how="left",
    )
    ...
    total_bioact = (
        best.groupby("Target_id")["lig_id"]
        .nunique()
        .reset_index()
        .rename(columns={"lig_id": "ChEMBL_bioactives_count"})
    )
    best = best[best["pX"] >= 8]
    total_potent = (
        best.groupby("Target_id")["lig_id"]
        .nunique()
        .reset_index()
        .rename(columns={"lig_id": "ChEMBL_bioactives_potent_count"})
    )
    ...
    if not best.empty:
        ...
        if not binding_data.empty:
            for name, group in binding_data.groupby("lig_id"):
                ...
                best_target_id = group.loc[group["association_prob"].idxmax()][
                    "Target_id"
                ]
                entropies.append(
                    {
            ...
            total_moderate_selectivity = (
                best[
                    (best["Selectivity"] <= 2)
                    & (best["best_target_id"] == best["chembl_target_id"])
                    & (best["number of other targets"] > 1)
                ]
                .groupby("Target_id")["lig_id"]
                .nunique()
                .reset_index()
                .rename(
            ...
            total_good_selectivity = (
                ...
                .nunique()
                .reset_index()
                .rename(columns={"lig_id": "ChEMBL_bioactives_good_selectivity_count"})
            )
            total_great_selectivity = (
                best[
                    (best["Selectivity"] <= 1)
                    & (best["best_target_id"] == best["chembl_target_id"])
                    & (best["number of other targets"] > 1)
                ...
                .nunique()
                .reset_index()
                .rename(columns={"lig_id": "ChEMBL_bioactives_great_selectivity_count"})
            )
        else:
            total_moderate_selectivity = pd.DataFrame(
                {"Target_id": [], "ChEMBL_bioactives_moderate_selectivity_count": []}
            )
            total_good_selectivity = pd.DataFrame(
                {"Target_id": [], "ChEMBL_bioactives_good_selectivity_count": []}
            )
            total_great_selectivity = pd.DataFrame(
                {"Target_id": [], "ChEMBL_bioactives_great_selectivity_count": []}
            )
    else:
        total_moderate_selectivity = pd.DataFrame(
            {"Target_id": [], "ChEMBL_bioactives_moderate_selectivity_count": []}
    ...
    results["phenotype"]["lethal"] = results["phenotype"]["Phenotype"].str.contains(
        "lethal|death", case=False
    )
    results["phenotype"]["normal"] = results["phenotype"]["Phenotype"].str.contains(
        "no abnormal phenotype detected", case=False
    )
    number_of_genotypes = (
        results["phenotype"]
        .groupby(["Target_id"])["genotype"]
        .nunique()
        .reset_index()
        .rename(columns={"genotype": "number_of_genotypes"})
    )
    pheno_hetero_lethal = (
        results["phenotype"][
            (results["phenotype"]["lethal"] == True)
            & (results["phenotype"]["zygosity"] == "HETEROZYGOTE")
        ]
        .groupby("Target_id")["Allele_symbol"]
        .count()
        .reset_index()
        .rename(columns={"Allele_symbol": "phenotypes_heterozygotes_lethal_count"})
    )
    pheno_homo_lethal = (
        results["phenotype"][
            (results["phenotype"]["lethal"] == True)
        ...
        .count()
        .reset_index()
        .rename(columns={"Allele_symbol": "phenotypes_homozygotes_lethal_count"})
    )
    pheno_hetero_normal = (
        results["phenotype"][
        ...
        .count()
        .reset_index()
        .rename(columns={"Allele_symbol": "phenotypes_heterozygotes_normal_count"})
    )
    pheno_homo_normal = (
        results["phenotype"][
            (results["phenotype"]["normal"] == True)
        ...
        .count()
        .reset_index()
        .rename(columns={"Allele_symbol": "phenotypes_homozygotes_normal_count"})
    )
    ...
    total_pdb = (
        ...
        .nunique()
        .reset_index()
        .rename(columns={"PDB_code": "PDB_total_count"})
    )
    pdb_w_lig = (
        results["pdb"][results["pdb"]["type_of_binder"].notnull()]
        .groupby("Target_id")["PDB_code"]
        .nunique()
        .reset_index()
        .rename(columns={"PDB_code": "PDB_with_Ligand_count"})
    )
    pdb_sites_tractable = (
        results["pdb"][results["pdb"]["SITES_tractable"] == 1]
        .groupby("Target_id")["PDB_code"]
        .nunique()
        .reset_index()
        .rename(columns={"PDB_code": "PDB_sites_tractable_count"})
    )
    pdb_sites_druggable = (
        results["pdb"][results["pdb"]["SITES_druggable"] == 1]
        .groupby("Target_id")["PDB_code"]
        .nunique()
        .reset_index()
        .rename(columns={"PDB_code": "PDB_sites_druggable_count"})
    )
    pdb_blast_close = (
        results["pdb_blast"]
        .groupby("Query_target_id")["PDB_code"]
        .nunique()
        .reset_index()
        .rename(
            columns={
                "PDB_code": "PDB_blast_close_count",
    ...
    pdb_blast_max_simil = (
        results["pdb_blast"]
        .groupby("Query_target_id")["similarity"]
        .max()
        .reset_index()
        .rename(
            columns={
                "similarity": "PDB_blast_max_similarity",
                "Query_target_id": "Target_id",
            }
        )
    )
    ...
    general_data = pd.read_sql(
        """SELECT Target_id,Sequence FROM Targets WHERE Target_id in ('%s')"""
        % target_id,
    ...
    results["domains"]["ranges"] = list(
        zip(results["domains"].start, results["domains"].stop)
    )
    domain_range = (
        results["domains"]
        .groupby("Target_id")["ranges"]
        .apply(lambda x: domain_ranges(x))
        .reset_index()
    )
    domain_range.index = domain_range.Target_id
    pdb_range = (
        results["pdb"]
        .groupby("Target_id")["start_stop"]
        .apply(lambda x: range_list(x))
        .reset_index()
    )
    ...
    for tid in pdb_range.index:
        ...
        pdb_range.loc[tid, ["%_sequence_covered"]] = np.where(
            (n_residues / general_data.loc[tid].length) > 1,
            1,
    ...
    data = data.merge(
        ...
        .count()
        .reset_index()
        .rename(columns={"start": "domains_count"}),
        on="Target_id",
        how="left",
    )
    data = data.merge(
        results["domain_drugE"].rename(
            columns={"tractable": "domain_tractable", "druggable": "domain_druggable"}
        ),
        on="Target_id",
        how="left",
    )
    ...
    data = data.merge(
        results["commercials"]
        .groupby("target_id")["smiles"]
        .count()
        .reset_index()
        .rename(columns={"smiles": "commercial_total", "target_id": "Target_id"}),
        on="Target_id",
        how="left",
    )
    data = data.merge(
        results["commercials"][results["commercials"]["affinity_value"] <= 100]
        .groupby("target_id")["smiles"]
        .count()
        .reset_index()
        .rename(
            columns={"smiles": "commercial_potent_total", "target_id": "Target_id"}
        ),
        on="Target_id",
        how="left",
    )
    ...
    tcrd_queries = {
        "target": """SELECT * FROM tcrd_target WHERE tcrd_id in ('%s') """
        % tcrd_id_list,
        "tdl_info": """SELECT * FROM tcrd_info WHERE protein_id in ('%s')"""
        % tcrd_id_list,
        "patent": """SELECT * FROM tcrd_patent where protein_id in ('%s')"""
        % tcrd_id_list,
    ...
    ORDER BY score DESC"""
        % tcrd_id_list,
        "novelty": """SELECT score,protein_id as tcrd_id FROM tcrd_novelty WHERE protein_id in ('%s')"""
        % tcrd_id_list,
    }
    tcrd_res = {
        qname: pd.read_sql(query, con=connector_targetDB)
        for qname, query in tcrd_queries.items()
    }
    ...
    tcrd_data = tcrd_data.merge(
        tcrd_res["patent"]
        .groupby("protein_id")["count"]
        .sum()
        .reset_index()
        .rename(columns={"count": "total_patent_count", "protein_id": "tcrd_id"}),
        on="tcrd_id",
        how="left",
    )
    tcrd_data = tcrd_data.merge(
        tcrd_res["patent"]
        .iloc[tcrd_res["patent"].groupby("protein_id")["count"].idxmax()]
        .rename(
            columns={
                "protein_id": "tcrd_id",
                "year": "year_max_patents",
                "count": "count_patents_max_year",
            }
        ),
        on="tcrd_id",
        how="left",
    ...
    disease_list = (
        disease_clean.groupby("protein_id")["name"]
        .unique()
        .apply("\n".join)
        .reset_index()
        .rename(columns={"protein_id": "tcrd_id", "name": "disease_list_tcrd"})
    )
    disease_count = (
        disease_clean.groupby("protein_id")["name"]
        .nunique()
        .reset_index()
        .rename(columns={"protein_id": "tcrd_id", "name": "disease_count_tcrd"})
    )
    disease_max = (
        disease_clean.loc[disease_clean.groupby("protein_id")["score"].idxmax().values]
        .drop(["parent", "doid", "disease_id"], axis=1)
        .rename(
            columns={
                "protein_id": "tcrd_id",
                "score": "max_disease_score",
                "name": "name_max_disease",
            }
        )
        .round(2)
    )
    ...
    tcrd_data = tcrd_data.merge(
        tcrd_res["novelty"].rename(columns={"score": "novelty_score"}),
        on="tcrd_id",
        how="left",
    )
